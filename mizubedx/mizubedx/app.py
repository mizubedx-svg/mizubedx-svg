...
def api_status():
    """現在の気象庁警報・気象実況（気温/湿度）・多摩川水位・直近24時間トレンド・危険度を返す。"""
    db = get_db()
    ...
    status["history_24h"] = external_data.get_history_24h(db)
        ...
        if area == "二子玉川バーベキュー場":
            ...
            db.execute(
                """INSERT INTO obs_bbq (
                    ...
                    temperature_snapshot_c, humidity_snapshot_pct
                ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (
                    ...
                    external_status.get("temperature_c"), external_status.get("humidity_pct"),
                ),
            )
...
# ------------------------------------------------------------------
# 日次PDFレポート（Gemini要約＋気象実況＋24時間トレンド＋写真埋め込み）
# ------------------------------------------------------------------
...
def api_report_pdf(observation_id):
    ...
    if area_row["external_snapshot_at"]:
        weather_context = {
            ...
            "temperature_c": area_row["temperature_snapshot_c"], "humidity_pct": area_row["humidity_snapshot_pct"],
        ...
        weather_context = {
            ...
            "temperature_c": live_status.get("temperature_c"), "humidity_pct": live_status.get("humidity_pct"),
            "fetched_at": live_status.get("fetched_at"), "is_snapshot": False,
        }

    # 直近24時間の推移は「PDF生成時点」の最新トレンドを表示する（観測時点の値を固定するものではない）
    history_24h = external_data.get_history_24h(db)
    ...
    pdf_bytes = report_generator.render_report_pdf(
        ...
        staff_summary=staff_summary, report_summary=report_summary, photo_data_uri=photo_data_uri,
        weather_context=weather_context, history_24h=history_24h,
    )
