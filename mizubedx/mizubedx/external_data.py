...
def get_external_status(db_conn, force_refresh=False):
    ...
    # --- ④ 気象実況（気温・湿度） ---
    temperature_c, humidity_pct, weather_code = None, None, None
    try:
        w = fetch_current_weather_openmeteo()
        temperature_c = w["temperature_c"]
        humidity_pct = w["humidity_pct"]
        weather_code = w["weather_code"]
    except Exception as e:
        print(f"[external_data] 気象実況取得失敗: {e}")
    ...
    db_conn.execute(
        """INSERT INTO external_status_cache
           (id, fetched_at, jma_area_code, jma_warnings_json, jma_report_time,
            ...
            temperature_c, humidity_pct, weather_code)
           VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    # 24時間トレンド用に、この取得結果を履歴テーブルへ蓄積（古い履歴は自動削除）
    _record_history_snapshot(db_conn, now_iso, river_level, discharge_m3s, temperature_c, humidity_pct)
...
# ------------------------------------------------------------------
# ④ 気象実況（気温・湿度）：Open-Meteo Weather Forecast API
#    （気象庁の警報・注意報APIには気温・湿度は含まれないため、
#     同じくスクレイピング不要の公式無料APIであるOpen-Meteoから取得する）
#
#    注意：Open-Meteo Flood API（河川データ）と違い、通常の気象APIは
#    時間単位（hourly）のデータを提供しているため、直近24時間の
#    気温・湿度トレンドはこの公式APIから直接取得可能。
# ------------------------------------------------------------------
OPEN_METEO_FORECAST_URL = "https://api.open-meteo.com/v1/forecast"


def fetch_current_weather_openmeteo():
    """二子玉川付近の現在の気温・湿度・天気コード、および直近24時間の
    気温・湿度の時系列（時間単位・実測に基づく直近データ）を取得する。

    戻り値: {
      "temperature_c": float|None, "humidity_pct": float|None, "weather_code": int|None,
      "hourly_temperature": [{"time": str, "value": float}, ...],
      "hourly_humidity": [{"time": str, "value": float}, ...],
    }
    """
    params = {
        "latitude": FUTAKO_LAT,
    ...
    resp = requests.get(OPEN_METEO_FORECAST_URL, params=params, timeout=8)
    resp.raise_for_status()
    data = resp.json()
    ...
    hourly_temperature = [{"time": t, "value": v} for t, v in zip(times, temps) if v is not None]
    hourly_humidity = [{"time": t, "value": v} for t, v in zip(times, humids) if v is not None]
...
# ⑤ 24時間トレンド用の履歴蓄積
#
#    河川流量シミュレーション(GloFAS)は日次分解能のため、外部APIだけでは
#    「24時間の滑らかな推移」は作れない。そのため、本システムが実際に
#    取得したスナップショットを external_status_history に蓄積し、
#    運用時間の経過に応じて自然にトレンドが埋まっていく方式にする。
#    （運用開始直後は点数が少なく、24時間運用後にトレンドが充実する）
# ------------------------------------------------------------------
HISTORY_RETENTION = timedelta(hours=48)  # 48時間より古い履歴は自動削除


def _record_history_snapshot(db_conn, fetched_at, river_level_m, river_discharge_m3s,
                               temperature_c, humidity_pct):
    db_conn.execute(
        """INSERT INTO external_status_history
           (fetched_at, river_level_m, river_discharge_m3s, temperature_c, humidity_pct)
           VALUES (?, ?, ?, ?, ?)""",
        (fetched_at, river_level_m, river_discharge_m3s, temperature_c, humidity_pct),
    )
    cutoff = (datetime.now() - HISTORY_RETENTION).isoformat(timespec="seconds")
    db_conn.execute("DELETE FROM external_status_history WHERE fetched_at < ?", (cutoff,))


def get_history_24h(db_conn):
    """直近24時間分の履歴を返す。
    温度・湿度は Open-Meteo の時間単位実測データ（即時に埋まる）、
    水位・流量は本システムが蓄積したスナップショット（運用時間に応じて埋まる）。
    """
    cutoff = (datetime.now() - timedelta(hours=24)).isoformat(timespec="seconds")
    rows = db_conn.execute(
        """SELECT fetched_at, river_level_m, river_discharge_m3s, temperature_c, humidity_pct
           FROM external_status_history WHERE fetched_at >= ? ORDER BY fetched_at ASC""",
        (cutoff,),
    ).fetchall()

    river_level = [{"time": r["fetched_at"], "value": r["river_level_m"]} for r in rows if r["river_level_m"] is not None]
    river_discharge = [{"time": r["fetched_at"], "value": r["river_discharge_m3s"]} for r in rows if r["river_discharge_m3s"] is not None]

    # 気温・湿度はOpen-Meteoの時間単位実測データを直接使う（自前蓄積より高精度・即時に埋まる）
    hourly_temperature, hourly_humidity = [], []
    try:
        w = fetch_current_weather_openmeteo()
        hourly_temperature = w["hourly_temperature"]
        hourly_humidity = w["hourly_humidity"]
    except Exception as e:
        print(f"[external_data] 気温・湿度の24時間履歴取得失敗: {e}")
