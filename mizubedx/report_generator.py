...
def generate_report_summary(area, score_items, danger_flags, site_memo, weather_context=None):
    ...
    weather_actual_text = (
        f"気温{temperature}℃・湿度{humidity}%" if temperature is not None and humidity is not None else "データなし"
    )
...
# ③ PDF生成
# ------------------------------------------------------------------
def _sample_history_for_pdf(series, max_points=6):
    """PDFの表に収まるよう、時系列データを均等に間引く（先頭・末尾は必ず含む）。"""
    if not series:
        return []
...
def _format_history_time(iso_str):
    """PDF表示用に時刻部分だけを短く整形する（YYYY-MM-DDTHH:MM -> MM/DD HH:MM）。"""
    try:
...
def render_report_pdf(observation, score_items, danger_flags, site_memo,
                       staff_summary, report_summary, photo_data_uri,
                       weather_context=None, history_24h=None):
    """Jinja2テンプレート(templates/report.html)にデータを流し込み、PDFバイト列を返す。

    history_24h: {"river_level": [...], "river_discharge": [...],
                   "temperature": [...], "humidity": [...]}
    それぞれ [{"time": iso文字列, "value": float}, ...] 形式。
    wkhtmltopdfでのグラフ描画（Chart.js等）は信頼性が低いため、
    PDF内は間引いた時系列を簡易表として印字する（詳細はdev memo参照）。
    """
    history_24h = history_24h or {}
    history_tables = {}
    for key in ("river_level", "river_discharge", "temperature", "humidity"):
        sampled = _sample_history_for_pdf(history_24h.get(key) or [])
        history_tables[key] = [
            {"time": _format_history_time(p["time"]), "value": p["value"]} for p in sampled
        ]
